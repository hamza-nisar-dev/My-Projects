...
def button(update, context):
  ...
  if a== '50a' or a=='25a' or a=="70a" or a=='125a' or a=="200a":#or a== '48' or a=='95' or a=="120"or a=='210' or a=="360" or a== '50' or a=='100' or a=="135"or a=='240' or a=="450":
    ...
    if inv !='0':
        ...
        if c==2:
            keyboard = [[InlineKeyboardButton("VIP SPOT", url="https://t.me/+TECfU89pyqZmODlh")],
    ...
    post=post['txid']
    keyboard = [[InlineKeyboardButton("I have Paid", callback_data='9zz')],[InlineKeyboardButton("Make a Payment", url=url)],
                [InlineKeyboardButton("🔙 Cancel", callback_data='6')]]
    reply_markup = InlineKeyboardMarkup(keyboard)
    user=int(update.effective_user.id)
    dat= date.today() + relativedelta(months=dd)
    conn = sqlite3.connect('sql.db')
    conn.execute("UPDATE COMPANY set Trx = '{}',exp='{}',amount='{}' where ID = {}".format(post,dat,pp,user))
    conn.commit()
    conn.close()
    ...
    if inv !='0':
        ...
        if c==2:
            ...
            reply_markup = InlineKeyboardMarkup(keyboard)
            conn = sqlite3.connect('sql.db')
            cursor = conn.execute("SELECT exp,refby,amount from COMPANY where ID= {}".format(update.effective_user.id))
            for names in cursor:
                inv=names[0]
                asf=names[1]
                cds=float(names[2])
    ...
    if inv !='0':
        ...
        c=transactionInfo['status']
        if c==2:
            keyboard = [[InlineKeyboardButton("VIP futuros", url="https://t.me/+etQDXTrpLTsyZTcx")],
                [InlineKeyboardButton("🔙 Main Menu", callback_data='6')]]    
            reply_markup = InlineKeyboardMarkup(keyboard)
            conn = sqlite3.connect('sql.db')
            cursor = conn.execute("SELECT exp from COMPANY where ID= {}".format(update.effective_user.id))
    ...
    post = {
        'txid' : transaction['txn_id'],}
    post=post['txid']
    keyboard = [[InlineKeyboardButton("I have Paid", callback_data='9vb')],[InlineKeyboardButton("Make a Payment", url=url)],
                [InlineKeyboardButton("🔙 Cancel", callback_data='6')]]
    reply_markup = InlineKeyboardMarkup(keyboard)
    user=int(update.effective_user.id)
    dat= date.today() + relativedelta(months=dd)
    ...
    if inv !='0':
        ...
        c=transactionInfo['status']
        if c==2:
            keyboard = [[InlineKeyboardButton("VIP ELITE", url="https://t.me/+NLkItvDdJXI0YWIx")],
                [InlineKeyboardButton("🔙 Main Menu", callback_data='6')]]    
            reply_markup = InlineKeyboardMarkup(keyboard)
            conn = sqlite3.connect('sql.db')
            cursor = conn.execute("SELECT exp from COMPANY where ID= {}".format(update.effective_user.id))
    ...
    post = {
        'txid' : transaction['txn_id'],}
    post=post['txid']
    keyboard = [[InlineKeyboardButton("I have Paid", callback_data='9gh')],[InlineKeyboardButton("Make a Payment", url=url)],
                [InlineKeyboardButton("🔙 Cancel", callback_data='6')]]
    reply_markup = InlineKeyboardMarkup(keyboard)
    user=int(update.effective_user.id)
    dat= date.today() + relativedelta(months=dd)
    ...
    if inv !='0':
        ...
        if c==2:
            keyboard = [[InlineKeyboardButton("Trading Signal Channel", url="https://t.me/+TECfU89pyqZmODlh")],[InlineKeyboardButton("Trading Signal Channel", url="https://t.me/+etQDXTrpLTsyZTcx")],
                [InlineKeyboardButton("🔙 Main Menu", callback_data='6')]]    
            reply_markup = InlineKeyboardMarkup(keyboard)
            conn = sqlite3.connect('sql.db')
            cursor = conn.execute("SELECT exp from COMPANY where ID= {}".format(update.effective_user.id))
    ...
    post = {
        'txid' : transaction['txn_id'],}
    post=post['txid']
    keyboard = [[InlineKeyboardButton("I have Paid", callback_data='9wp')],[InlineKeyboardButton("Make a Payment", url=url)],
                [InlineKeyboardButton("🔙 Cancel", callback_data='6')]]
    reply_markup = InlineKeyboardMarkup(keyboard)
    user=int(update.effective_user.id)
    dat= date.today() + relativedelta(months=dd)
    ...
    if inv !='0':
        ...
        c=transactionInfo['status']
        if c==2:
            keyboard = [[InlineKeyboardButton("VIP SPOT", url="https://t.me/+TECfU89pyqZmODlh")],[InlineKeyboardButton("VIP futuros", url="https://t.me/+etQDXTrpLTsyZTcx")],
                [InlineKeyboardButton("🔙 Main Menu", callback_data='6')]]    
            reply_markup = InlineKeyboardMarkup(keyboard)
            conn = sqlite3.connect('sql.db')
            cursor = conn.execute("SELECT exp,refby,amount from COMPANY where ID= {}".format(update.effective_user.id))
...
def callback_minut(context):
    ...
    if da !="0":
        ...
        if x>y:
            ...
            conn.close()
            try:
              context.bot.kick_chat_member(chat_id=-1001194157852,user_id=int(idd))
            except:
                context.bot.kick_chat_member(chat_id=-1001314484558,user_id=int(idd))
