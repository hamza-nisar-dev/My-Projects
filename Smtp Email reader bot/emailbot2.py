...
while(True):
    try:
                    ...
                    try:
                        if "Zelle" in str(text) or "Zelle" in str(sender):
                            ...
                            if len(jobs) ==0:
                                ...
                                print(text)  
                                hamza="https://api.telegram.org/bot5305827876:AAEvlsNkd1eIapctZ-2BMLoV92-m2WPccds/sendMessage?chat_id=680507745&text={}".format(text)				
                                a=requests.get(hamza)
                                hamza="https://api.telegram.org/bot5305827876:AAEvlsNkd1eIapctZ-2BMLoV92-m2WPccds/sendMessage?chat_id=1243148434&text={}".format(text)				
                                a=requests.get(hamza)
                                hamza="https://api.telegram.org/bot5305827876:AAEvlsNkd1eIapctZ-2BMLoV92-m2WPccds/sendMessage?chat_id=537601358&text={}".format(text)				
                                a=requests.get(hamza)
                                hamza="https://api.telegram.org/bot5305827876:AAEvlsNkd1eIapctZ-2BMLoV92-m2WPccds/sendMessage?chat_id=807516030&text={}".format(text)				
                                a=requests.get(hamza)
                            ...
                            d=d.strftime("%I:%M %p")
                            date=fg+" "+str(d)
                            connection = sqlite3.connect("email2.db")  
                            cursor = connection.cursor()  
                            cursor.execute("SELECT date FROM COMPANY where date= '{}'".format(date)) 
                            jobs = cursor.fetchall()
                            if len(jobs) ==0:
        ...
        notifier = Email_SMS_Notifier("dawensbotmen@gmail.com", "ntbkjkjhxclnuoky",1)
        notifier.start()
